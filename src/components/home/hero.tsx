...
export function Hero() {
  return (
    <section className="relative overflow-hidden border-b border-white/10">
      <div className="pointer-events-none absolute inset-0 grid-bg opacity-15" aria-hidden />
      ...
      <div className="container-shell relative z-10 grid min-h-[calc(100vh-5rem)] items-center gap-14 py-20 lg:grid-cols-[1.12fr_0.88fr]">
        <div>
          ...
          >
            <Link href="/projects" className="btn-cta-primary" data-cursor-hover>
              View Projects <ArrowRight size={16} />
            </Link>
            <Link href="/contact" className="btn-cta-secondary" data-cursor-hover>
              Contact Me
            </Link>
          </motion.div>
        </div>
        ...
        >
          ...
          <div className="mt-6 space-y-4">
            {[
              ...
              <div
                key={item.title}
                className="rounded-2xl border border-white/10 bg-white/5 p-4 transition duration-500 ease-out hover:border-sky-400/20 hover:bg-white/[0.07]"
              >
